     ...
     if(editable && editable._id==project._id){
        return (
            <div className='flex flex-col gap-4 p-4 border border-gray-300 rounded-md'>
                ...
                return(<div>{project[key]!==""? <input key={i} onChange={async(e)=>setEditable({...editable,[key]: key=="img"?await handleFileChange(e.target.files[0],setPreview):e.target.value})} type={key=='img'?'file':'text'} value={key=="img"?'':editable[key]} />:''}
